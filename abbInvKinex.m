...
% 10920730
% MEGN 544
function [th1, th2, th3, th4, th5, th6, reach]  = abbInvKinex(T_des, th_last)
...
for j = 1:6
    ...
    for k = 1:8
        ...
        %
        s = abs(th_last(j) - thetaW(j, k));
        s_p = abs(th_last(j) - thetaW(j, k) + pi);
        s_m = abs(th_last(j) - thetaW(j, k) - pi);

        %
        if(s_p < s) % bin 1
            thetaW(j, k) = thetaW(j, k) - pi;
        elseif(s_m < s) % bin 2
            thetaW(j, k) = thetaW(j, k) + pi;
        end
    end
